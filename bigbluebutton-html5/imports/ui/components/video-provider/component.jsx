...
import PopupVideoListContainer from '../popup-video-list/container';
...
class VideoProvider extends Component {
  constructor(props) {
    ...
    this.blockStream = this.blockStream.bind(this);
    ...
    this.handleNewVideoInPopup = this.handleNewVideoInPopup.bind(this);
  }
  ...
  componentDidMount() {
    ...
    window.addEventListener('blockStream', this.blockStream);
    window.addEventListener('restartStream', this.restartStream);

    window.addEventListener('addStreamToPopupWindow', this.handleNewVideoInPopup);
  ...
  restartStream(e){
    ...
    if(correspondingStream != null){
      //Restart stream
      this.handleReceivedStream(correspondingStream, true)
    }
  }
  ...
  onWsClose() {
    ...
    const removeStreamFromPopupWindow = new CustomEvent('removePopupVideo', { detail: { mediaElement: cameraId } });
    window.dispatchEvent(removeStreamFromPopupWindow);
  ...
  connectStreams(streamsToConnect) {
    streamsToConnect.filter(cId => !this.state.hiddenStreams.includes(cId)).forEach((cameraId) => {
  ...
  disconnectStreams(streamsToDisconnect) {
    streamsToDisconnect.forEach(cameraId => {
      ...
      // const handleStreamEvent = new CustomEvent('streamShown', { detail: { mediaElement: cameraId } });
      // window.dispatchEvent(handleStreamEvent);
      // const handleAudioEvent = new CustomEvent('audioOn', { detail: { mediaElement: cameraId } });
      // window.dispatchEvent(handleAudioEvent);
    });
  }
  ...
  stopWebRTCPeer(cameraId, restarting = false, removeFromState = true) {
    ...
    var currentStreams = popupStreams;
    currentStreams.forEach(s => {
      if(s.cameraId.toString().startsWith(cameraId.toString())){
        let index = currentStreams.indexOf(s)
        currentStreams.splice(index,1)
      }
    })

    if(currentStreams.length === 0 && this.externalWindow != null){
      this.externalWindow.close()
    } else {
      const removeStreamFromPopupWindow = new CustomEvent('removePopupVideo', { detail: { mediaElement: cameraId } });
      window.dispatchEvent(removeStreamFromPopupWindow);
    ...
    this.destroyWebRTCPeer(cameraId, removeFromState);

    this.setState({ shownStreams: currentStreams })
  }

  destroyWebRTCPeer(cameraId, removeFromState = false) {
    const peer = this.webRtcPeers[cameraId];
    if (peer) {
      logger.info({
  ...
  async createWebRTCPeer(cameraId, isLocal) {
    ...
    let cameraIdentifier = cameraId.slice();
    cameraIdentifier = cameraIdentifier.substr(15, cameraIdentifier.length+1)
  ...
  handleNewVideoInPopup(e){
    ...
    if(popupStreams.length === 0){
      ...
      this.externalWindow = window.open('', "popupWindow", "width="+screen.availWidth+",height="+screen.availHeight)
      ...
      this.setState({ popupStreams: currentPopupStreams }, () => {
        if(streamAlreadyAdded){
          //rimuovi stream
          const removeVideoEvent = new CustomEvent('removePopupVideo', { detail: { mediaElement: mediaElement.toString() } });
          ...
          //aggiungi stream
          const addStreamToPopupWindow = new CustomEvent('addPopupVideo', { detail: { mediaElement: mediaElement.toString() } });
  ...
  render() {
    ...
    streams.forEach(stream => {
      let found = false
      hiddenStreams.forEach(s => {
        if (stream.cameraId.toString().startsWith(s.toString())) {
          found = true
        }
      })
      if(!found)
        actualStreams.push(stream)
    })
